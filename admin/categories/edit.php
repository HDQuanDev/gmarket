...
if (isset($_POST['submit'])) {
    ...
    if (empty($path)) {
        ...
        $path_check = fetch_array("SELECT COUNT(*) as count FROM categories WHERE path='$path' AND id != '$id'");
        if ($path_check && $path_check['count'] > 0) {
            $path = $path . '-' . rand(100, 999);
        }
    }
    
    $update_query = "UPDATE categories SET name='$name', path='$path'";
        ...
        if (array_key_exists($ext, $allowed)) {
            ...
            if (move_uploaded_file($_FILES['banner']['tmp_name'], $path_banner)) {
                if (!empty($category['img'])) {
                    $old_path = $_SERVER['DOCUMENT_ROOT'] . "/public/uploads/all/" . $category['img'];
                    if (file_exists($old_path)) {
                        @unlink($old_path);
                    }
                }
                $update_query .= ", img='$newname'";
            } else {
...
<head>
    ...
    <title>Takashimaya | Edit Category</title>
            ...
            <div class="aiz-main-content">
                <div class="px-15px px-lg-25px">
                    <div class="aiz-titlebar text-left mt-2 mb-3">
                        <h5 class="mb-0 h6"><?=tran("Edit Category")?></h5>
                    </div>
                            ...
                            <div class="card-body">
                                ...
                                <form class="form-horizontal" action="edit.php?id=<?=$id?>" method="POST" enctype="multipart/form-data">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                            <input type="text" placeholder="<?=tran("Name")?>" name="name" class="form-control" required value="<?=$category['name']?>">
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                            <input type="text" placeholder="<?=tran("Path")?>" name="path" class="form-control" value="<?=$category['path']?>">
                                            <small class="text-muted"><?=tran("URL path for this category.")?></small>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                            ...
                                            <?php if(!empty($category['img'])): ?>
                                                <div class="mt-2">
                                                    <img src="/public/uploads/all/<?=$category['img']?>" alt="Current Banner" class="img-fluid" style="max-width: 300px;">
                                                    <p class="text-muted mb-0"><?=tran("Current Banner")?></p>
                                                </div>
                                            <?php endif; ?>
                                        </div>
                                    </div>
