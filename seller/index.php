...
$sql = "SELECT SUM(admin_commission) as total_commission
        FROM commission_history as c
        WHERE c.seller_id = ?";
...
$total_sales = $res->fetch_assoc()['total_commission'] ?? 0;
...
$stmt = $conn->prepare("SELECT SUM(`commission_history`.`seller_earning`) as total FROM commission_history WHERE seller_id=? AND created_at >= ?");
$stmt->bind_param("is", $seller_id, $current_month);
$stmt->execute();
$stmt->bind_result($total);
$stmt->fetch();

$current_month_sales = floatval($total ?? 0);
$stmt->close();
}catch(Exception $e) {
    dd($e);
}
