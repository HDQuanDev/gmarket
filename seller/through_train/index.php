...
<body class="">
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="col-xl-8 mx-auto text-center">
                                    <h1 class="mb-0 fw-700"><?=tran("Premium Packages for Sellers")?></h1>
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="py-4 py-lg-5">
                        <div class="container">
                            <div class="row row-cols-xxl-4 row-cols-lg-3 row-cols-md-2 row-cols-1 gutters-10 justify-content-center">
                                <?php
                                $sql=mysqli_query($conn,"SELECT * FROM through_trains ");
                                while($row=fetch_assoc($sql)){
                                    $logo=fetch_array("SELECT * FROM file WHERE id='{$row['logo']}' LIMIT 1");
                                ?>
                                <div class="col">
                                    <div class="card overflow-hidden">
                                        <div class="card-body">
                                            <div class="text-center mb-4 mt-3">
                                                <img class="mw-100 mx-auto mb-4"  src="/public/<?=$logo?'uploads/all/'.$logo['src']:'assets/img/placeholder.jpg'?>"
                                                    height="100">
                                                <h5 class="mb-3 h5 fw-600"><?=$row['name']?></h5>
                                            </div>
                                            <ul class="list-group list-group-raw fs-15 mb-5">
                                                <li class="list-group-item py-2 flex" style="min-width:260px">
                                                    <i class="las la-check text-success mr-2"></i>
                                                    Nạp <?=currency($row['amount'])?>$ tặng <b><?=$row['people']?>$</b>
                                                </li>
                                            </ul>
                                            <div class="mb-5 d-flex align-items-center justify-content-center">
                                                <span
                                                    class="fs-30 fw-600 lh-1 mb-0"><?=currency($row['amount'])?>$</span>
                                            </div>

                                            <div class="text-center">
                                                <button class="btn btn-primary fw-600"
                                                    onclick="select_payment_type(<?=$row['id']?>)"><?=tran("Purchase Package")?></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><?=tran("Select Payment Type")?></h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form class="" id="package_payment_form" action="/seller/offline-seller-through-train-paymnet"
                        method="post">
                        <input type="hidden" name="_token" value="jeVFiapLZrmTexe2gwykiL1aVi2OH3FnaNVdk7e4"> <input type="hidden" id="through_train_id" name="through_train_id" value="">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="mb-3">
                                    <select class="form-control aiz-selectpicker">
                                        <option value="">Use Wallet Balance ($<?=currency($seller_money)?>)</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                        <div class="form-group text-right">
                            <button type="button" class="btn btn-sm btn-primary transition-3d-hover mr-1"
                                id="select_type_cancel" data-dismiss="modal"><?=tran("Cancel")?></button>
                            <button type="submit"
                                class="btn btn-sm btn-primary transition-3d-hover mr-1"><?=tran("Confirm")?></button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <div class="modal fade" id="price_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form class="" id="through_train_payment_form" action="/seller_through_train/purchase"
                    method="post">
                    <input type="hidden" name="_token" value="jeVFiapLZrmTexe2gwykiL1aVi2OH3FnaNVdk7e4"> <input type="hidden" name="seller_through_train_id" value="">
    ...
    <div class="modal fade" id="offline_seller_through_train_purchase_modal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div id="offline_seller_through_train_purchase_modal_body"></div>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        function select_payment_type(id) {
            $('input[name=through_train_id]').val(id);
            $('#select_payment_type_modal').modal('show');
        }

        function payment_type(type) {
            var through_train_id = $('#through_train_id').val();
            if (type == 'online') {
                $("#select_type_cancel").click();
                show_price_modal(through_train_id);
            } else if (type == 'offline') {
                $("#select_type_cancel").click();
                $.post('/seller/offline-seller-through-train-purchase-modal', {
                    _token: 'jeVFiapLZrmTexe2gwykiL1aVi2OH3FnaNVdk7e4',
                    through_train_id: through_train_id
                }, function(data) {
                    $('#offline_seller_through_train_purchase_modal_body').html(data);
                    $('#offline_seller_through_train_purchase_modal').modal('show');
                });
            }
        }

        function show_price_modal(id) {
            $('input[name=seller_through_train_id]').val(id);
            $('#price_modal').modal('show');
        }

        function get_free_through_train(id) {
            $('input[name=seller_through_train_id]').val(id);
            $('#through_train_payment_form').submit();
        }
    </script>
