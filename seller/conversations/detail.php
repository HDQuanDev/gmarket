...
$conversasion=fetch_array("SELECT * FROM conversations WHERE id='{$_GET['id']}' LIMIT 1");

if(!$conversasion)die();

$p=fetch_array("SELECT * FROM products WHERE id='{$conversasion['product_id']}' LIMIT 1");

$user=fetch_array("SELECT * FROM users WHERE id='{$conversasion['user_id']}' LIMIT 1");
...
<body class="">
                ...
                <div class="px-15px px-lg-25px">
                    <div class="aiz-titlebar mt-2 mb-4">
                        <div class="h6">
                            ...
                            <a href="/shop/Shop-V%C3%A2n-Nhi-sotre-555" class="" translate="no"><?=$seller_shop_name?></a>
                        </div>
                    </div>
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title fs-16 fw-600 mb-0">#<?=$p['name']?>
                                (
                                Between you and
                                <?=$user['full_name']?>
                                )
                        ...
                        <div class="card-body">
                            <ul class="list-group list-group-flush">
                                ...
                                while($row=fetch_assoc($sql)){
                                    if($row['type']=='user'){
                                        ...
                                        $sender="<str style='color:green'>".$seller_shop_name."</str>";
                                    }
                                ...
                                <li class="list-group-item px-0">
                                    <div class="media mb-2">
                                        ...
                                        <img class="avatar avatar-xs mr-3" src="/public/assets/img/placeholder.jpg" onerror="this.onerror=null;this.src='/public/assets/img/avatar-place.png';">
                            ...
                            <?php
                            if(isset($_POST['send_new_reply'])){
                                $message=$_POST['message'];
                                @mysqli_query($conn,"INSERT into conversation_details(seller_id,user_id,message,type,conversation_id,create_date)values('$seller_id','{$user['id']}','$message','seller','{$_GET['id']}','$createDate')");
                                echo "<script>window.location.href=''</script>";

                            }
                            ?>
                            <form class="pt-4" action="" method="POST">
                                <input type="hidden" name="_token" value="sY3tzsrJ2efBsz6gfCOaAFhO1F38qubVTunRAMoB"> <input type="hidden" name="conversation_id" value="917">
                                <div class="form-group">
                                    <textarea class="form-control" rows="4" name="message" placeholder="Type your reply" required></textarea>
                                </div>
                                <div class="form-group mb-0 text-right">
                                    <button type="submit" name="send_new_reply" class="btn btn-primary">Send</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        function refresh_messages() {
            $.post('/seller/conversations/refresh', {
                _token: 'sY3tzsrJ2efBsz6gfCOaAFhO1F38qubVTunRAMoB',
                id: 'eyJpdiI6Ik9JNFJBd3hPNmJ1ZUY2dFFKMnFYVmc9PSIsInZhbHVlIjoiNFZQZDlWREVmckowNjZSWjFiTnN5dz09IiwibWFjIjoiODNiYjViOGRiMDc2YTY2ZDAyNDExNjE1YzM5ODU3Zjg1ZGYwNjA5NzY3YjJiNmFhMzgxZWMyMjAzNjllNWQzOCIsInRhZyI6IiJ9'
            }, function(data) {
                $('#messages').html(data);
            })
        }
