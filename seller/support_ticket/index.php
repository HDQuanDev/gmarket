...
<head>
    <meta name="csrf-token" content="VoszYTrT5jAV44lQYyZ8sVEOYJ6mTda1IJ34W3un">
                ...
                <div class="px-15px px-lg-25px">
                    <div class="aiz-titlebar mt-2 mb-4">
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                <h1 class="h3"><?=tran("Support Ticket")?></h1>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <table class="table aiz-table mb-0">
                                <thead>
                                    <tr>
                                        <th data-breakpoints="lg"><?=tran("Ticket ID")?></th>
                                        <th data-breakpoints="lg"><?=tran("Sending Date")?></th>
                                        <th><?=tran("Subject")?></th>
                                        <th><?=tran("Status")?></th>
                                        <th data-breakpoints="lg"><?=tran("Options")?></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    $sql=mysqli_query($conn,"SELECT * FROM supports WHERE seller_id=$seller_id ORDER by id desc ");
                                    ...
                                    <tr>
                                        ...
                                        <td><?=$row['subject']?></td>
                                        <td>
                                            <span class="badge badge-inline badge-<?=$row['status']=='Pending'?"success":"danger"?>"><?=$row['status']?></span>
                                        </td>
                                        <td>
                                            <a href="/seller/support_ticket/detail.php?id=<?=$row['id']?>" class="btn btn-styled btn-link py-1 px-0 icon-anim text-underline--none">
                                            <?=tran("View Details")?>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title strong-600 heading-5"><?=tran("Create a Ticket")?></h5>
                ...
                <div class="modal-body px-3 pt-3">
                    ...
                    if(isset($_POST['send_ticket'])){
                        ...
                        $title=$_POST['subject'];
                        $code="@".$seller_id.time();
                        @mysqli_query($conn,"INSERT into supports(code,seller_id,subject,last_reply,create_date)values('$code','$seller_id','$title','$createDate','$createDate')");
                        $last_id = mysqli_insert_id($conn);
                        @mysqli_query($conn,"INSERT into support_details(seller_id,support_id,type,details,files,create_date)values('$seller_id','$last_id','seller','$details','$files','$createDate')");
                        echo "<script>alert('Gửi thành công');window.location.href=''</script>";
                    }
                    
                    ?>
                    <form class="" action="" method="post" enctype="multipart/form-data">
                        <input type="hidden" name="_token" value="VoszYTrT5jAV44lQYyZ8sVEOYJ6mTda1IJ34W3un">
                        <div class="row">
                            <div class="col-md-2">
                                <label><?=tran("Subject")?></label>
                            </div>
                            <div class="col-md-10">
                        ...
                        <div class="row">
                            <div class="col-md-2">
                                <label><?=tran("Provide a detailed description")?></label>
                            </div>
                            <div class="col-md-10">
                        ...
                        <div class="text-right mt-4">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal"><?=tran("Cancel")?></button>
                            <button type="submit" class="btn btn-primary" name="send_ticket"><?=tran("Send Ticket")?></button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        if ($('#lang-change').length > 0) {
            $('#lang-change .dropdown-menu a').each(function() {
                $(this).on('click', function(e) {
                    ...
                    $.post('/language', {
                        _token: 'VoszYTrT5jAV44lQYyZ8sVEOYJ6mTda1IJ34W3un',
                        locale: locale
                    }, function(data) {
                        location.reload();
                    });
