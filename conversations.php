...
if(isset($_POST['product_id'])){
    ...
    $checkExsit=fetch_array("SELECT  * FROM conversations WHERE seller_id='$seller' and product_id='$product_id' LIMIT 1");
    if($checkExsit){

    }
    else{
        @mysqli_query($conn,"INSERT into conversations(seller_id,user_id,title,product_id,create_date)values('$seller','$user_id','$title','$product_id','$createDate')");
        $conversation_id=mysqli_insert_id($conn);

        @mysqli_query($conn,"INSERT into conversation_details(message,seller_id,user_id,conversation_id,create_date)values('$message','$seller','$user_id','$conversation_id','$createDate')");
