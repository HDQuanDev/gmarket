      ...
      <section class="content">
        ...
        <div class="card">
          <div class="card-header">
            <h3 class="card-title">Duyệt cộng tiền</h3>
            ...
            <table class="table table-striped projects" id="table1">
              ...
              <?php
                if(isset($_POST['success'])){
                  $id=$_POST['id'];
                  $user_nap=$_POST['user_nap'];
                  $money=$_POST['money'];
                  @mysqli_query($conn,"UPDATE users SET money=money+$money WHERE id=$user_nap LIMIT 1");
                  @mysqli_query($conn,"UPDATE history_payment SET status='Success'  WHERE id=$id  LIMIT 1");

                  echo "<script>alert('Nạp thành công');window.location.href='./'</script>";
                }
                if(isset($_POST['error'])){
                  $id=$_POST['id'];
                  $user_nap=$_POST['user_nap'];
                  @mysqli_query($conn,"UPDATE history_payment SET status='Error'  WHERE id=$id  LIMIT 1");
                  echo "<script>alert('Huỷ thành công');window.location.href='./'</script>";
                }
              
                  $l=mysqli_query($conn,"SELECT * FROM history_payment  WHERE type='Recharge' ORDER by id DESC");
                  $i=0;
                  while($lenh=mysqli_fetch_assoc($l)){
                    $user_nap=fetch_array("SELECT * FROM users WHERE id='{$lenh['user_id']}' LIMIT 1");
                ...
                <tr class="text-center">
                  ...
                  <td>
                    <b><?=$user_nap['email']?></b>
                    <input type="text" name="user_nap" hidden value="<?=$user_nap['id']?>">
                  </td>
            
               
                  <td>
                    <input type="number" name="money"  hidden value="<?=$lenh['amount']?>" id="">
                    <b><?=($lenh['amount'])?></b>
                  </td>
                  ...
                  <td class="project-actions text-center" style="min-width: 160px;">
                    <!-- xác nhận -->
                    <?php if($lenh['status']=='Pending'){?>
                   <button type="submit" name="success" class="btn btn-success btn-sm confirm-btn"><i class="fa fa-check"></i></button>
                    <!-- ? -->
                    <!-- <a class="btn btn-info btn-sm" href="#"><i class="fas fa-pencil-alt"></i></a> -->
                    <!-- Xóa -->
                   <button type="submit" name="error" class="btn btn-danger btn-sm delete-btn"><i class='fa fa-close'></i></button>
                   <?php }?>
                  </td>
                </tr>
                </form>
